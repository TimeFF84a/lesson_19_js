...
function checkDomainUrl(str) {
  return (/^(https|http):\/\/([a-z0-9_-]+\.)*[a-z0-9][a-z0-9_-]+\.[a-z]{2,4}$/).test(str);
}
...
function createLinksFromDomains(str) {
  str.match(/(https|http):\/\/([a-z-]+\.)*[a-z][a-z]+\.[a-z]{2,4}/gi).forEach(e => str = str.replace(e, `<a href="${e}" target="_blank">${e.replace(/^(http:|https:)\/\//gi, '')}</a>`));
